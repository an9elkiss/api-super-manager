...
public class FileTreeServiceImpl implements FileTreeService{
    ...
    public ApiResponseCmd<Object> saveFileTree(FileTreeNode fileTree){
        ...
        if (null == fileTree){
            LOGGER.info(" id为{}，姓名为  {} 的用户，提交的需要新建的文件内容为空", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName());
        ...
        if (null == fileTree.getParentId()){
            LOGGER.info(" id为{}，姓名为  {} 的用户，提交新建的节点id的父级节点id为空", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName(), fileTree.getId());
        ...
        if (StringUtils.isEmpty(fileTrees)){
            LOGGER.info(" id为{}，姓名为  {} 的用户，提交新建的节点id的父级节点id节点无信息", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName());
        ...
        if (fileTrees.getFileType().equals(ApiStatus.FILE_TREE_FILE)){
            LOGGER.info(" id为{}，姓名为  {} 的用户，所添加的文件的父节点的文件类型为文件，不允许添加", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName());
        ...
        LOGGER.info(" id为{}，姓名为  {} 的用户，创建节点为{}", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName(), fileTree);
    ...
    public ApiResponseCmd<List<FileTreeNode>> findAllFileTrees(){
        ...
        if (fileTrees.isEmpty()){
            ...
            fileTree.setUpdateBy(AppContext.getPrincipal().getName());
            fileTreeDao.save(fileTree);
            LOGGER.info(" id为{}，姓名为  {} 的用户，创建父节点 id 为{}", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName(), fileTree.getId());
    ...
    public ApiResponseCmd<FileTreeNode> findFileTree(Integer id){
        ApiResponseCmd cmd = new ApiResponseCmd<>();
        if (null == id){
            LOGGER.info(" id为{}，姓名为  {} 的用户，请求的参数为空", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName());
            cmd.setCode(ApiStatus.FILE_TREE_OPERATE_ERROR.getCode());
            cmd.setMessage(ApiStatus.FILE_TREE_OPERATE_ERROR.getMessage());
            return cmd;
        }
        ...
        if (StringUtils.isEmpty(fileTrees)){
            LOGGER.info(" id为{}，姓名为  {} 的用户，请求的参数为无法查询到节点树", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName());
            cmd.setCode(ApiStatus.FILE_TREE_OPERATE_ERROR.getCode());
            cmd.setMessage(ApiStatus.FILE_TREE_OPERATE_ERROR.getMessage());
            return cmd;
        }
    ...
    public ApiResponseCmd<String> uploadFile(MultipartFile multipartFile){
        ...
        // 文件上传服务器地址
        String fileUrl = String.format("%4$s%1$tY%1$tm%1$te%2$s%3$s", new Date(), String.valueOf(new java.util.Random().nextInt()), multipartFile.getOriginalFilename(), "static/");
        // 文件
    ...
    public ApiResponseCmd<Map<String, List<Integer>>> checkoutPreMonthHeartSound(String token){
        ...
        try{
            ...
            LOGGER.error(" id为{}，姓名为  {} 的用户，提交查询报表时，系统发送服务器请求失败请求地址{}", AppContext.getPrincipal().getId(), AppContext.getPrincipal().getName(), URL);
        }catch (IOException e){
    ...
    private void findSubordinateByLeaderid(List<UserPersonCmd> userPersonCmds,Map<Integer, UserPersonCmd> userPersonCmdMap,Map<Integer, List<UserPersonCmd>> leadMap) {
